...
from sections.TracepointV3 import trace_point_log_type_ctors
...
class FirehoseChunk(TraceV3Chunk):
    def __init__(self, tracev3_fd: io.BufferedReader):
        ...
        self.has_private_data = self.metadata.private_data_offset != 4096
        self.private_data_size = 4096 - self.metadata.private_data_offset
        self.trace_point_offset = self.fd.tell()
        self.private_data_real_offset: int = self.trace_point_offset + self.metadata.public_data_size
    ...
    def read_trace_points(self):
        ...
        i = 1
        while self.fd.tell() < trace_points_start + self.metadata.public_data_size:
